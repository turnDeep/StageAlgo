...
class GammaPlotter:
    ...
    def plot_analysis(self):
        ...
        plt.tight_layout()
        filename = f"{self.ticker.lower()}_gamma_analysis.png"
        plt.savefig(filename)
        print(f"Chart saved to {filename}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Gamma Profile & Historical Risk Plotter")
    parser.add_argument("-t", "--ticker", type=str, required=True, help="Stock Ticker Symbol")
    args = parser.parse_args()

    plotter = GammaPlotter(args.ticker)
    if plotter.fetch_data():
        plotter.calculate_current_gamma_levels()
        plotter.calculate_historical_metrics()
