...
def fetch_stock_data(ticker: str, benchmark_ticker: str = "SPY", period: str = "2y"):
    ...
    # curl-cffiの'impersonate'機能がここで重要です。
    ...
    try:
        ...
        data = yf.download(
            ...
            period=period,
            session=session,
            progress=False  # 出力をクリーンに保つため
        )

        if data.empty or ticker not in data.columns.get_level_values(1):
            print(f"エラー: {ticker} のデータをダウンロードできませんでした。")
            return None, None
        ...
        if stock_data.empty:
             print(f"エラー: クリーニング後、{ticker} の有効なデータがありません。")
             return None, None

        return stock_data, benchmark_data

    except Exception as e:
        print(f"{ticker} のデータ取得中に例外が発生しました: {e}")
        return None, None
