...
class TimeSeriesQuantLibAnalyzer:
    ...
    def plot_analysis(self):
        ...
        for i in range(len(self.hist) - 1):
            ...
            color = 'green' # Low Vol
            if hv > 50: color = 'yellow' # Normal
            if hv > 80: color = 'red' # High Vol
        ...
        plt.tight_layout()
        plt.savefig(f"{self.ticker.lower()}_timeseries_analysis.png")
        print(f"Chart saved to {self.ticker.lower()}_timeseries_analysis.png")
    ...
    def generate_report(self):
        ...
        print(f"\n=== {self.ticker} Time-Series Analysis Report ===")
        print(f"Current HV: {hv_now:.2f}% (Range: {hv_min:.2f}% - {hv_max:.2f}%)")
        print(f"Current Downside Bias: {bias:.2f} pts")

        if hv_now < hv_min + (hv_max - hv_min) * 0.2:
            cycle = "Contraction (Base Forming)"
        elif hv_now > hv_max * 0.8:
            cycle = "Expansion (Climax/Panic)"
        else:
            cycle = "Transition"
...
if __name__ == "__main__":
    analyzer = TimeSeriesQuantLibAnalyzer("HL")
