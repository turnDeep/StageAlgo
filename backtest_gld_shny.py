...
def main():
    ...
    trade_ticker = "SHNY"  # Direxion Daily Gold Bull 2X Shares

    # Note: SHNY inception date is around 2024. If 4 years of history is requested,
    ...
    simulation_df['SHNY_Open'] = trade_data['Open']
    simulation_df['SHNY_Close'] = trade_data['Close']
    ...
    # Strategy 1: UT Bot on GLD -> Trade SHNY
    capital = 10000.0
    shares = 0
    in_position = False
    ...
    # Initial Entry
    if initial_pos == 1:
        buy_price = test_data.iloc[0]['SHNY_Open']
    ...
    for i in range(len(test_data) - 1):
        ...
        next_open = test_data.iloc[i+1]['SHNY_Open']
        ...
        current_equity = cash + (shares * test_data.iloc[i]['SHNY_Close'])
        equity_curve.append(current_equity)

    last_price = test_data.iloc[-1]['SHNY_Close']
    final_equity_strat = cash + (shares * last_price)
    equity_curve.append(final_equity_strat)

    # Strategy 2: Buy and Hold SHNY
    bh_buy_price = test_data.iloc[0]['SHNY_Open']
